...
def test_hyperparameter_space_defaults():
    ...
    assert space.lr_max == 3e-3
    assert space.weight_decay_min == 1e-7
    assert 32 in space.batch_sizes
    assert "sqrt_inverse" in space.class_weights_options
    assert space.allow_clahe is False


@pytest.mark.parametrize(("permitido", "esperado_en_la_busqueda"), [(False, False), (True, True)])
...
def test_sample_parameters():
    ...
    assert 1e-5 <= params["learning_rate"] <= 3e-3
...
def test_export_tuning_artifacts(tmp_path: Path):
    ...
    def dummy_objective(trial: optuna.Trial) -> float:
        ...
        bs = objective.sample_parameters(trial)["batch_size"]
    ...
    assert (output_dir / "figures/optimization_history.png").exists()

    with open(output_dir / "study_summary.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    assert data["model"] == "efficientnet_b0"
    assert data["n_recorded"] == 3
