...
Responder siempre \texttt{healthy}, la clase m\'as frecuente con 1~311 de las 5~015 im\'agenes, ya acierta el \textbf{26.14\%} sin mirar un p\'ixel. Ese predictor constante es el control nulo y la columna de exceso mide lo que cada condici\'on a\~nade sobre \'el.

Al tapar el centro, donde el encuadre habitual del corpus sit\'ua la l\'amina foliar, el modelo conserva el \textbf{79.46\%}, esto es \textbf{$+53.32$ puntos sobre el nulo}.

Queda establecido que \textbf{la regi\'on exterior porta por s\'i sola una fracci\'on sustancial de la informaci\'on de clase}, pero no que el fondo pese m\'as que la hoja: las dos condiciones no est\'an igualadas en \'area, ya que el anillo muestra 1.75 veces m\'as p\'ixeles que la caja central. La comparaci\'on rigurosa es la de cada condici\'on contra el nulo, y ambas lo superan, la ca\'ida de 18.45 puntos al tapar el centro demuestra que este aporta se\~nal propia.

Cada repositorio se fotografi\'o con su propio suelo, luz y sensor, y cuatro de las catorce fuentes contienen una sola clase: \textbf{la periferia no predice la enfermedad, predice el repositorio, y el repositorio acota la enfermedad}. Es lo mismo que arroja desde otro \'angulo la Tabla~\ref{tab:kfold_results}, donde el Macro $F_1$ cae a 0.6026 al evaluar fuentes nunca vistas. 

Con esto se puede concluir que en una parcela salvadore\~na el suelo y la luz no se parecen a los del corpus, de modo que esa evidencia deja de ayudar y puede confundir.

Recortar el fondo mediante un segmentador autom\'atico a negro degrad\'o el Macro $F_1$ a 0.7191, esta ca\'ida mezcla dos efectos: la p\'erdida del atajo de procedencia, y los errores de la propia segmentaci\'on, que sobre clases de fondo poco contrastado rechaza o recorta tejido leg\'itimo.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.70\linewidth]{images/fairness/gradcam_samples.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\linewidth]{images/xai/panel_compare_common_rust.png}
...
El perfil se calcul\'o sobre 270 im\'agenes (30 por clase), empleando como m\'ascara de hoja la segmentaci\'on producida por el modelo del repositorio \texttt{maize-doctor-segmenter}. Las tres clases restantes —\textit{gusano cogollero}, \textit{necrosis letal} y \textit{deficiencia de potasio}— quedan fuera de la tabla porque su m\'ascara (debido a los límites del segmentador entrenado) alcanza coberturas de entre 0.54 y 0.86 y el control de calidad rechaza entre el 37\% y el 57\% de sus im\'agenes, de modo que su ratio no es interpretable.
...
La viabilidad de una soluci\'on de inteligencia artificial en el sector agropecuario no se mide \'unicamente por su puntuaci\'on de F1, sino por su capacidad para transformar las condiciones de vida de los sectores m\'as vulnerables y encajar en las pol\'iticas p\'ublicas del pa\'is.

\subsection{Recomendaciones de pol\'itica p\'ublica aplicables}

Para traducir los avances t\'ecnicos de este proyecto en acciones, se proponen cinco medidas de pol\'itica p\'ublica dirigidas al Ministerio de Agricultura y Ganader\'ia (MAG) y al CENTA:

\subsubsection{1. Adopci\'on a futuro de DoctorMaiz como herramienta oficial de extensi\'on}
El MAG puede homologar la aplicaci\'on m\'ovil como herramienta de apoyo digital de bolsillo para los m\'as de 300 extensionistas y promotores agr\'icolas comunitarios del CENTA. Su funcionamiento offline permite que el t\'ecnico emita diagn\'osticos estandarizados en caser\'ios remotos de Caba\~nas o Moraz\'an sin consumir datos m\'oviles, complementando el juicio agron\'omico presencial y reduciendo el tiempo de atenci\'on.
...
La auditor\'ia de procedencia de este proyecto demostr\'o que entrenar con fotos de \'Africa o India limita la generalizaci\'on en El Salvador. El Estado puede financiar brigadas de captura fotogr\'afica participativa, articuladas con estudiantes de la Universidad de El Salvador (UES) y escuelas agr\'icolas, para nutrir un repositorio soberano de im\'agenes de variedades locales (como el h\'ibrido H-59 y ma\'ices criollos), recalibrando peri\'odicamente los modelos para cerrar la brecha de dominio que ha sido el principal hallazgo y limitaci\'on de la Etapa 2.
...
\begin{enumerate}
\item \textbf{Superaci\'on de metas de desempe\~no:} El ensamble heterog\'eneo por voto suave alcanz\'o un Macro $F_1$ de \textbf{0.9567} y una exactitud de \textbf{98.29\%}, superando con holgura el umbral de viabilidad del proyecto ($F_1 \geq 0.85$). 
...
\begin{itemize}
\item \textbf{Piloto de validaci\'on en cooperativas salvadore\~nas:} Desplegar la aplicaci\'on en campo con agricultores evaluando la adopci\'on y usabilidad real del marco gu\'ia en condiciones de luz solar intensa.
...
\item \textbf{Anotaci\'on de severidad para predecir el grado de da\~no:} Calificar un subconjunto del corpus con las escalas diagram\'aticas homologadas, de la mano de fitopat\'ologos del CENTA, abrir\'ia la puerta a una cabeza de regresi\'on ordinal capaz de estimar el nivel adem\'as de la clase.
\item \textbf{Modelos en cascada con segmentaci\'on liviana en servidor:} Evaluar en el backend en la nube la incorporaci\'on de un detector ligero (YOLOv8-Nano) que a\'isle la hoja antes del ensamble cuando se reciban im\'agenes con encuadres lejanos, para intentar mejorar la robustez frente a fotos de campo con mucho fondo.
\end{itemize}
