...
La configuraci\'on \'optima obtenida (Trial \#12) demostr\'o una \textbf{mejora significativa frente al baseline de esta nueva etapa (con el dataset completo)} sobre el conjunto de prueba independiente (Tabla~\ref{tab:opt_vs_base}).
...
\textbf{El fondo por s\'i solo predice mejor que la hoja por s\'i sola (79.46\% vs. 42.25\%)}. Al tapar la hoja entera, la red retiene el 69\% de su confianza y acierta casi 8 de cada 10 veces, apoy\'andose en el suelo o la iluminaci\'on. 
...
\item \textbf{Gu\'ia de severidad en lenguaje llano:} El clasificador responde \textit{qu\'e} tiene la hoja, no \textit{cu\'anto}. Sus salidas son una clase y una confianza, y ninguna mide \'area foliar afectada, n\'umero de lesiones ni grado de defoliaci\'on: dos hojas con tres p\'ustulas y con la l\'amina destruida reciben el mismo diagn\'ostico y una confianza parecida. No se entren\'o un modelo de niveles de da\~no porque el corpus consolidado trae una sola etiqueta por imagen y ninguna anotaci\'on de severidad; producirla no es trabajo de limpieza sino de calificaci\'on experta, pues cada pat\'ogeno se punt\'ua con su propia escala diagram\'atica validada (Cobb modificada para roya, Vieira \textit{et al.} para tiz\'on del norte, Brito \textit{et al.} para mancha gris, la ordinal 1--5 del CIMMYT para necrosis letal y la 1--9 de Davis \textit{et al.} para cogollero), y partir en tres grados clases que rondan las 300 im\'agenes habr\'ia dejado celdas de dos d\'igitos, insuficientes para entrenar y para evaluar. Para cubrir ese vac\'io sin fingir una medici\'on, debajo de cada resultado de inferencia la app despliega los niveles de severidad de la clase diagnosticada traducidos a lenguaje llano —cu\'anta hoja compromete cada uno, qu\'e se ve y qu\'e hacer seg\'un manejo integrado de plagas— y advierte de forma expl\'icita: \textbf{«La app no mide qu\'e tan avanzado est\'a el da\~no»}. El contenido reescribe el cat\'alogo de escalas homologadas por CIMMYT, Embrapa, Iowa State University Extension e IPNI, conservando los umbrales num\'ericos y los nombres de los bioinsumos pero eliminando la nomenclatura binomial y las siglas patom\'etricas. El grado lo juzga el agricultor comparando su hoja, y esa elecci\'on no se registra ni se presenta como predicci\'on del modelo.
\end{enumerate}
...
\item \textbf{Prototipo m\'ovil de alta eficiencia:} Se logr\'o compilar \texttt{EfficientNet-Lite0} a Int8 ocupando apenas \textbf{3.56~MB}, con una latencia de inferencia de \textbf{60~ms} en un tel\'efono f\'isico Motorola edge 40 neo, resolviendo el encuadre mediante un marco gu\'ia foliar, protegiendo la salida con un detector de anomal\'ias OOD por distancia de Mahalanobis y complementando la etiqueta con una gu\'ia de severidad en lenguaje llano que suple, en la interfaz, la ausencia de anotaciones de nivel de da\~no en el corpus.
\end{enumerate}
...
\item \textbf{Anotaci\'on de severidad para predecir el grado de da\~no:} Calificar un subconjunto del corpus con las escalas diagram\'aticas homologadas, en coordinaci\'on con fitopat\'ologos del CENTA, y entrenar una cabeza de regresi\'on ordinal que estime el nivel adem\'as de la clase. La gu\'ia de autoevaluaci\'on pasar\'ia entonces de sustituto a explicaci\'on de la predicci\'on.
